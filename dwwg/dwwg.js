...
v2p重写
安卓APP:匹配链接（正则表达式）：http://xud66.com.xiud66.top/member/bin.php?
IOS微信登陆：匹配链接（正则表达式）：http://xud66.com.xiud66.top/member/bin.php?
...
                console.log(`【金币余额】： ${data} 元 \n`)
                if(data >= 1.1){
                    await $.wait(3000)
                    console.log(`【提现检测】：达到最低提现额度，自动提现\n`);
                    $.message += `【提现检测】：达到最低提现额度，自动提现\n`
                    dwwgtx()
                }else{
