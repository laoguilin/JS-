...
let rw = {
    ...
    CX12001: "添加监控", CX12002: "添加关注", CX12005: "分享任务", CX12007: "高级搜索", CX12011: "点赞观点"
  };
...
!(async () => {
    if (typeof $request !== "undefined") {
        ...
        console.log(
            ...
            console.log(`共${aqcckArr.length}个账号`);
        for (let i = 0; i < aqcckArr.length; i++) {            
            if (aqcckArr[i]) {                
                aqcck = aqcckArr[i];
                headers.cookie = aqcck;
                alltaskList = [];
                ytaskList = [];
                taskList = [];
                claimList = [];
                $.index = i + 1;
                console.log(`查询第${$.index}个账号信息\n`);
                await aqccxrw(`${$.index}`)
                await $.wait(3000)
